...
parser.add_argument(
    "--portal_data",
    type=str,
    default="data/2023_portal_time_features_sjoin_2023_filtered.csv",
)
parser.add_argument(
    "--inrix_data",
    type=str,
    default="data/2023_inrix_time_features_sjoin_2023_filtered.csv",
)
parser.add_argument(
...
assert mode in (
    ...
    "mask",
    "zfill_mask",
), "mode must be 'zfill', 'mask', or 'zfill_mask'"
...
def to_tensor(df, device="cuda", dtype=np.float64):
    ...
    arr = np.ascontiguousarray(df.to_numpy(dtype=dtype, copy=False))
...
if mode == "zfill":
    ...
    initial_mmd2 = mmd_torch_masked(
        X, Mx, Y, My, gamma=gamma, rescale_by_d=True, use_valid_pair_denominators=True
    )
...
if save:
    ...
    mmd2_vals_filename = f"{"+".join([Path(f).stem for f in travel_time_data_files])}-{mode}-{n_permutations}_perms-{timestamp_string}.csv"
