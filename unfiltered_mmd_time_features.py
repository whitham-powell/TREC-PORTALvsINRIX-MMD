...
It creates pivoted data from unfiltered time series on the fly, then computes MMD² using
three missing data strategies sequentially.
...
Output:
    ...
    {year}-time_features_unfiltered-{mode}-{n_perms}_perms-{timestamp}.csv
"""
...
parser.add_argument(
    "--year", type=int, default=2023, help="Year of data to process, e.g., 2023"
)
...
P_time_features_unfiltered = (
    portal_ts_df_unfiltered.pivot(
        index="stationid", columns="portal_tstamp", values="portal_tt"
    )
    .reindex(columns=time_grid)  # align to full-year grid
    .sort_index(axis=1)
    .astype(np.float64)
)
...
print("inrix...")
I_time_features_unfiltered = (
    inrix_ts_df_unfiltered.pivot(
        index="tmc_code", columns="inrix_tstamp", values="inrix_tt"
    )
    .reindex(columns=time_grid)  # align to full-year grid
    .sort_index(axis=1)
    .astype(np.float64)
...
portal_df = P_time_features_unfiltered
inrix_df = I_time_features_unfiltered
...
for mode in ["zfill", "mask", "zfill_mask"]:
    ...
    if save:
        ...
        mmd2_vals_filename = f"{ts_year}-time_features_unfiltered-{mode}-{n_permutations}_perms-{timestamp_string}.csv"
